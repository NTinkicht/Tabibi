...
The scoped coding/testing adapter from merged PR #359 can run only on a fresh owner-issued exact-head sole-implementer lease with PAYG disabled, after the WU115 reviewed per-lease activation change merges. A live coding capability proof still remains necessary. Its model child proposes bounded edits only; an isolated trusted parent validates lease, paths, exact SHA and tests and alone commits/pushes a canonical PR. Never describe the review-only wake as a coding runtime, and never claim operational Mistral coding until a real model-authored feature/test commit has green 3/3 current-SHA CI and a non-Mistral final gate.
