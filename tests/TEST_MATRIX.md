...
| --- | --- | --- | --- | --- |
| Clinic-scoped staff auth and no unauthenticated operational access | Unit, API, Integration | `tests/unit/staff-auth.test.ts`, `tests/api/session-operations.test.ts`, `tests/integration/queue-operations-negative.test.ts` | `CI / Quality and build`, `CI / PostgreSQL integration` | Covered |
| Same-origin mutation enforcement | API, Integration | `tests/api/session-operations.test.ts`, `tests/integration/queue-operations-negative.test.ts` | `CI / Quality and build`, `CI / PostgreSQL integration` | Covered |
...
| Tenant isolation and role denial on queue operations | Integration | `tests/integration/walkin-queue.test.ts`, `tests/integration/queue-priority.test.ts`, `tests/integration/queue-operations-negative.test.ts` | `CI / PostgreSQL integration` | Covered |
...
| `CLAUDE-032` | `no_show` requires an audited reason | `tests/integration/queue-priority.test.ts`, `tests/integration/queue-operations-negative.test.ts` | Fixed and covered |
