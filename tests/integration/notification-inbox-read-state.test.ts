...
describe('in-app notification inbox read state', () => {
  it('marks one exact-subject item read and preserves the first timestamp on retry', async () => {
    ...
    const first = await repository.markRead({ ...ownScope, itemId: created.id });
    expect(first?.readAt).toEqual(expect.any(String));

    const second = await repository.markRead({ ...ownScope, itemId: created.id });
    expect(second?.readAt).toBe(first?.readAt);

    const listed = await repository.listForSubject({ ...ownScope, limit: 10 });
