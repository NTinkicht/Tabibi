...
describe('notification retry recovery', () => {
  it(
    'clears a scheduled retry deadline when a newer intent supersedes it',
    async () => {
  ...
  it(
    'restores retry eligibility when a failed claim is explicitly released',
    async () => {
      const repository = new NotificationOutboxRepository(pool);
  ...
  it(
    'recovers an expired non-final retry claim behind deterministic backoff',
    async () => {
      const repository = new NotificationOutboxRepository(pool);
  ...
  it(
    ...
    async () => {
      ...
      const afterReleases = await pool.query<{
        dispatch_attempt_count: number;
      }>(
