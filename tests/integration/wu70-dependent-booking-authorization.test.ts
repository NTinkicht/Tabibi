...
beforeEach(async () => {
  ...
  await pool.query(
    `TRUNCATE appointment_booking_receipts, appointments, audit_events, queue_entries, patient_dependents, patient_operational_records, consultation_sessions, schedule_templates, doctor_clinics, doctor_profiles, clinic_memberships, clinics, users CASCADE`,
  );
  await pool.query(
    `INSERT INTO users(id,auth_subject,display_name) VALUES ($1,'wu70-owner','Owner'),($2,'wu70-other-owner','Other Owner'),($3,'wu70-doctor','Doctor')`,
    [ids.owner, ids.otherOwner, ids.doctorUser],
  );
  await pool.query(`INSERT INTO clinics(id,tenant_key,name) VALUES($1,'wu70-clinic','WU70 Clinic')`, [
    ids.clinic,
  ]);
  await pool.query(`INSERT INTO clinic_memberships(clinic_id,user_id,role) VALUES($1,$2,'receptionist')`, [
    ids.clinic,
    ids.owner,
  ]);
  await pool.query(`INSERT INTO doctor_profiles(id,user_id,display_name) VALUES($1,$2,'Dr WU70')`, [
    ids.doctor,
    ids.doctorUser,
  ]);
  await pool.query(`INSERT INTO doctor_clinics(clinic_id,doctor_id) VALUES($1,$2)`, [ids.clinic, ids.doctor]);
  await pool.query(
    `INSERT INTO consultation_sessions (id,clinic_id,doctor_id,service_date,starts_at,ends_at,status) VALUES($1,$2,$3,'2026-09-16','2026-09-16 09:00Z','2026-09-16 12:00Z','open')`,
    [ids.session, ids.clinic, ids.doctor],
  );
  await pool.query(
    `INSERT INTO patient_operational_records (id,clinic_id,account_user_id,private_display_name,contact_phone,preferred_locale) VALUES($1,$2,$3,'Owner Patient','0555007001','fr')`,
    [ids.patient, ids.clinic, ids.owner],
  );
  await pool.query(
    `INSERT INTO patient_dependents (id,owner_user_id,display_name,status,archived_at) VALUES ($1,$4,'ليان Élodie','active',NULL),($2,$5,'Other Child','active',NULL),($3,$4,'Enfant archivé','archived',now())`,
    [ids.activeDependent, ids.crossAccountDependent, ids.archivedDependent, ids.owner, ids.otherOwner],
  );
});
afterAll(async () => {
  await closePool();
...
function authCookie() {
  return `tabibi_staff_session=${createStaffSessionToken('wu70-owner', new Date(Date.now() + 60_000))}`;
}
async function routeBooking(dependentId: string, idempotencyKey: string) {
  return appointmentRoute(
    new Request(`http://localhost/api/clinics/${ids.clinic}/sessions/${ids.session}/appointments`, {
      method: 'POST',
      headers: {
    ...
    { params: Promise.resolve({ clinicId: ids.clinic, sessionId: ids.session }) },
  );
}
async function bookingSinkCounts() {
  const result = await pool.query<{ entries: string; appointments: string; receipts: string; audits: string }>(
...
describe('WU70 dependent booking authorization boundary', () => {
  it('books one active owned dependent and persists only an internal durable reference', async () => {
    ...
    const audit = await pool.query<{ metadata: Record<string, unknown> }>(
      `SELECT metadata FROM audit_events WHERE clinic_id=$1 AND entity_type='appointment' AND entity_id=$2`,
  ...
  it('fails malformed, unknown, cross-account and archived dependent references identically with zero writes', async () => {
    ...
    for (const [index, dependentId] of dependentIds.entries()) {
      ...
      failures.push({ status: response.status, error: body.error, message: body.message });
    }
  ...
  it('fails closed when archive wins the row race before dependent authorization', async () => {
    ...
    try {
      ...
      await archiver.query(
        `UPDATE patient_dependents SET status='archived', archived_at=now(), updated_at=now() WHERE id=$1 AND owner_user_id=$2`,
      ...
      await expect(bookingPromise).rejects.toBeInstanceOf(AppointmentConflictError);
      expect(await bookingSinkCounts()).toEqual({ entries: '0', appointments: '0', receipts: '0', audits: '0' });
    } finally {
  ...
  it('preserves idempotent at-most-one booking under concurrent retries', async () => {
    ...
    expect(await bookingSinkCounts()).toEqual({ entries: '1', appointments: '1', receipts: '1', audits: '1' });
  });
