...
describe('receptionist dashboard concurrency snapshot', () => {
  ...
  it(
    'keeps doctor-delay version and ETA bounds on one committed PostgreSQL snapshot',
    async () => {
      const queue = new QueueService(pool);
  ...
  it(
    'keeps priority reorder and patients-ahead values on one committed PostgreSQL snapshot',
    async () => {
      const queue = new QueueService(pool);
