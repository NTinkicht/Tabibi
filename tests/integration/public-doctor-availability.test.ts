...
describe('public doctor availability', () => {
  it(
    'returns deterministic future bookable session windows without leaking internal identity or mutating state',
    async () => {
      ...
      expect(
        await service.listForDoctor(inactiveClinic, hiddenDoctor),
      ).toEqual([]);
