...
for (const browserLanguage of ['ar-DZ', 'AR_ae']) {
  test(`Arabic browser ${browserLanguage} opens public directory in RTL without language click`, async ({
    page,
  }) => {
...
test('Non-Arabic browser uses French directory and supports deliberate Arabic choice', async ({
  page,
}) => {
  ...
  await page.getByRole('button', { name: 'تحديث' }).click();
  await expect(page.locator('main[lang="ar"][dir="rtl"]')).toBeVisible();
  expect(await page.locator('main').innerText()).not.toContain(PRIVATE_MARKER);
});
