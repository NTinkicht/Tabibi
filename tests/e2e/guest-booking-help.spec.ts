...
test('guest booking help exposes French preparation guidance without private data', async ({
  page,
}) => {
  ...
  await expect(
    french.getByRole('link', { name: 'Comprendre les coordonnées requises' }),
...
test('guest booking help exposes equivalent Arabic RTL preparation guidance', async ({
  page,
}) => {
  await page.goto('/guest/booking-help');
  ...
  await expect(
    arabic.getByRole('link', { name: 'فهم بيانات التواصل المطلوبة' }),
  ).toHaveAttribute('href', '/guest/booking-contact-help');
