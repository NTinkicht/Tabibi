...
test('guest notification help is bilingual and privacy-minimal', async ({
  page,
}) => {
  ...
  await expect(
    arabic.getByRole('heading', { name: 'فهم إشعارات طبيبي' }),
  ).toBeVisible();
  await expect(
    french.getByRole('link', { name: 'Revenir au statut' }),
