...
function repoPath(requested) {
  ...
  if (sensitivePatterns.some((pattern) => pattern.test(relative))) {
    throw new Error(`Sensitive path is not eligible for context routing: ${relative}`);
  }
...
function search(query) {
  ...
  const lines = (result.stdout || "")
    .split(/\r?\n/)
    .filter(Boolean)
    .filter((line) => !sensitivePatterns.some((pattern) => pattern.test(line.split(":", 1)[0])))
...
function slice(requested, rawStart, rawCount = "120") {
  ...
  if (!Number.isSafeInteger(start) || start < 1) throw new Error("start-line must be >= 1");
...
function readBudget() {
  if (process.env.TABIBI_CONTEXT_ALLOW_COPILOT !== "1") {
    throw new Error("Copilot compression is disabled. Set TABIBI_CONTEXT_ALLOW_COPILOT=1 only after confirming included allowance and paid overage is disabled.");
  }
  if (!fs.existsSync(BUDGET_PATH)) {
    throw new Error("Missing .tabibi/context-budget.json; compression fails closed when local budget state is unknown.");
  }
  const budget = JSON.parse(fs.readFileSync(BUDGET_PATH, "utf8"));
  if (budget.hardStop === true) throw new Error("Local Copilot context hard stop is active.");
...
function parseSpec(spec) {
  ...
  if (end < start || end - start + 1 > MAX_SLICE_LINES) {
    throw new Error(`Compression slice must be <= ${MAX_SLICE_LINES} lines: ${spec}`);
  }
...
function buildEvidence(specs) {
  ...
  for (const spec of specs) {
    ...
    chunks.push(`FILE ${relative} lines ${start}-${Math.min(end, lines.length)}\n${excerpt}`);
  }
  if (!chunks.length) throw new Error("No eligible evidence excerpts were supplied.");
...
function compress(question, specs) {
  ...
  if (result.status !== 0) {
    throw new Error(result.stderr?.trim() || "Copilot compression failed; no paid/provider fallback is permitted.");
  }

  const nextBudget = { ...budget, remainingUnits: budget.remainingUnits - 1 };
  fs.mkdirSync(path.dirname(BUDGET_PATH), { recursive: true });
  fs.writeFileSync(BUDGET_PATH, `${JSON.stringify(nextBudget, null, 2)}\n`, "utf8");
...
try {
  ...
  console.error(`context-router: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(2);
}
