...
Gemini CLI may use the owner's configured free/non-billable allowance only. Mistral Vibe may use the owner's existing subscription allowance only, with PAYG disabled. Credentials never become repository assets or general-purpose workflow credentials. `GEMINI_API_KEY` remains limited to the dedicated owner-only Issue #11 wake in `.github/workflows/gemini-cli-wake.yml` when `TABIBI_GEMINI_ZERO_BILLING_CONFIRMED=true`. `MISTRAL_API_KEY` may be used only by `.github/workflows/mistral-vibe-wake.yml` for read-only review/analysis or `.github/workflows/mistral-scoped-code-adapter.yml` for the separate DEFAULT-OFF parent-controlled code-proposal lane, both when `TABIBI_MISTRAL_PAYG_DISABLED_CONFIRMED=true`. The review workflow may wake either from an owner Issue #11 dispatch or automatically after a same-repository PR's exact-head CI succeeds, but the automatic lane is valid only when the reviewed head uses the exact `.github/workflows/ci.yml` blob trusted on `main`; otherwise it fails closed. The model child remains read-only and receives no PR-write token. A separate trusted publisher may copy a sealed exact-head PASS and create a native approval only after rechecking current head, provenance and trusted 3-job CI. The scoped code adapter still activates **only for one owner-issued Issue #11 exact-head code dispatch** with a current canonical sole-implementer PR lease; no global code enable variable is needed. Vibe child can only read; trusted parent applies allow-listed code+tests, runs checks and pushes fast-forward after verifying exact SHA. No other unattended credential route is authorized; Mistral cannot merge or self-gate. Grok uses only included SuperGrok Grok Build or Grok Bot capacity (separate included weekly pools). Build uses owner-private OAuth; Bot uses owner-linked app sessions on its shared cloud computer. No `XAI_API_KEY`, extra usage credits/PAYG or shared OAuth tokens. Scoped exception: `.github/workflows/savegrok-cloud-slack-bridge.yml` may send metadata-only owner-lease signals through the ALREADY-authorized Slack relay after current-head 3/3 CI and supersession checks, and only if `TABIBI_GROK_CLOUD_BRIDGE_ENABLED=true` is deliberately set following owner-linked Bot/Slack/GitHub setup. It NEVER runs Grok itself or accesses Grok credentials, and cannot make cloud execution a verified capability without a Codespace-off review proof.
