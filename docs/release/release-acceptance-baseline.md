...
| Accessibility / keyboard navigation on release-critical receptionist and patient/public flows | PASS | WU48 `tests/e2e/receptionist-bilingual-accessibility.spec.ts`; WU49 bilingual full-day/public acceptance | Keyboard and semantic-state assertions must remain on the same product paths used by the release rehearsal. |
